...
context=stateTransitions.MultiContext()

def sendCommand( command, sequence ) :
	for processName in sequence :
		context.processes[processName].sendCommand(command)

def marksStopRun() :
	sendCommand( "stop", ["GlibStreamer"] )
	sendCommand( "Stop", ["GlibSupervisor","TrackerManager","StorageManager","otherFUEventProcessor","myFUEventProcessor","FUResourceBroker"] )
	
def marksStartRun() :
	sendCommand( "Enable", ["TrackerManager","StorageManager","otherFUEventProcessor","myFUEventProcessor","FUResourceBroker","GlibSupervisor"] )
	sendCommand( "start", ["GlibStreamer"] )
...
s.CreateExecutives()
s.Initialise()
supervisor.configure(triggerRate=1)
s.Configure()
streamer.configure(numberOfEvents=25)
s.Enable()

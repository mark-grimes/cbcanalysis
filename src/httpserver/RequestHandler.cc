...
httpserver::RequestHandler::RequestHandler()
	: doc_root_("/home/xtaldaq")
{
}

void httpserver::RequestHandler::handleRequest( const http::server::request& req, http::server::reply& rep )
{
	// Decode url to path.
...
bool httpserver::RequestHandler::url_decode( const std::string& in, std::string& out )
{
	out.clear();
	out.reserve( in.size() );

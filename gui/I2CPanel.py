...
class I2CPanel :
	...
	def echoSelection(self): #fb - a good "print screen" method
		msg = " Buttons clicked: "
		for names in self.getCheckedRegisters():
			msg+=self.getCheckedRegisters()
	...
	def getCheckedRegisters(self):
		selectedRegisters = []
		for name in getCheckedStates:
			if [name] == "Main Control":
				selectedRegisters += self.mainControlRegNames
			if name == "Channel Masks":
				selectedRegisters += self.channelMaskRegNames
			if name == "Channel Trims":
				selectedRegisters += self.channelTrimRegNames	
